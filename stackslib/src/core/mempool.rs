...
impl MemPoolDB {
    ...
    pub fn get_all_txs(conn: &DBConn) -> Result<Vec<MemPoolTxInfo>, db_error> {
        ...
        let rows = query_rows::<MemPoolTxInfo, _>(conn, &sql, rusqlite::params![])?;
        Ok(rows)
    }
    ...
    pub fn garbage_collect_tx_blacklist<'a>(
        ...
        let sz = query_int(tx, sql, rusqlite::params![])? as u64;
        if sz > max_size {
    ...
    pub fn dump_txs(&self) {
        let sql = "SELECT * FROM mempool";
        let txs: Vec<MemPoolTxMetadata> = query_rows(&self.db, sql, rusqlite::params![]).unwrap();
    ...
    pub fn get_max_height(conn: &DBConn) -> Result<Option<u64>, db_error> {
        ...
        let count = query_rows::<i64, _>(conn, sql, rusqlite::params![])?.len();
        if count == 0 {
            Ok(None)
        } else {
            let sql = "SELECT MAX(height) FROM mempool";
            Ok(Some(query_int(conn, sql, rusqlite::params![])? as u64))
        }
    }
